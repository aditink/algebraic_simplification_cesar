...
fn get_runner(has_node_limit: bool, timeout: u64) -> Runner<PropLang, ()> {
    // Default runner
...
/// # Returns
///
/// A `String` of the simplifed formula.
///
...
pub fn simplify(
    ...
    let mut runner = get_runner(has_node_limit, timeout);

    // Simplify the problem using the given rules.
    runner.with_expr(&problem).run(&rules);

    // Helps to extract the most simplifed formula from the graph.
    ...
    debug!("Cost of simplification: {}", simplifed.0);
