...
impl Simplification for  Pass1 {
    ...
    fn make_rules() -> Vec<Rewrite<PropLang, ()>> {
        ...
        fn _cell_collapse_guard(var_common: Var, var_cond: Var, var_predicate: Var) -> impl Fn(&mut EGraph<PropLang, ()>, Id, &Subst) -> bool {
            move |egraph, _, subst| {
        ...
        fn cell_collapse_guard_fn(var_common: Var, var_x: Var, var_y: Var, var_predicate: Var, cond_fml: impl Fn(String, String)->String) -> impl Fn(&mut EGraph<PropLang, ()>, Id, &Subst) -> bool {
            move |egraph, _, subst| {
                ...
                let predicate_fml = extractor.find_best(predicate).1.to_string();
                let assumptions = unsafe { ASSUMPTIONS.clone() };
                z3utils::imply(format!("(and {} {})", cond_fml, assumptions), format!("(= {} {})", common_fml, predicate_fml))
            }
        }
        ...
        fn no_divide_by_0(var_x: Var, var_y: Var, var_b: Var) -> impl Fn(&mut EGraph<PropLang, ()>, Id, &Subst) -> bool {
            move |egraph, _, subst| {
                let x = subst[var_x];
                let y = subst[var_y];
                let extractor = Extractor::new(&egraph, AstSize);
                ...
                let divide_by_x: Pattern<PropLang> = format!("(^ {} (-1)))", x_trm).parse().unwrap();
